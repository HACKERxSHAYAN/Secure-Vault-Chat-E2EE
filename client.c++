...
static_assert(sizeof(CryptoEngine::ROTATION_TABLE) == 256, "Rotation table must be 256 bytes");

// ──────────────────────────────────────────────────────────────
// CLIENT INTERFACE
// ──────────────────────────────────────────────────────────────

// Thread: Receive and decrypt messages from server
...
// Thread: Encrypt and send messages to server
void send_thread(socket_t sock) {
    ...
    while (running) {
        ...
        try {
            // Encrypt message before sending to network
...
╚═══════════════════════════════════════════════════════════╝
    ...
    // TCP/IP Protocol Hardening: Disable Nagle's algorithm
    int tcp_nodelay = 1;
    ...
    // Start receiver and sender threads (full duplex)
